-- TRANSFORM.DEV.FACT_APARTMENTIQ_UNITKPI_BH — UNITKPI_BH × UNIT_BH × PROPERTY_BH + HUD ZIP spine + **REFERENCE.GEOGRAPHY** TIGER.
-- Same enrichment as ``fact_apartmentiq_propertykpi_bh``.

{{ config(
    alias='fact_apartmentiq_unitkpi_bh',
    materialized='view',
...
FROM {{ source('transform_apartmentiq', 'unitkpi_bh') }} AS k
LEFT JOIN {{ source('transform_apartmentiq', 'unit_bh') }} AS u
    ON k.UNITID = u.ID
