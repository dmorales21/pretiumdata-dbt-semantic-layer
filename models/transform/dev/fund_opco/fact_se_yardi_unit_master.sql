-- TRANSFORM.DEV.FACT_SE_YARDI_UNIT_MASTER — read-through SOURCE_ENTITY.PROGRESS.YARDI_UNIT
-- Unit master from SOURCE_ENTITY (Sensitivity); not UNIT_PROGRESS silver.
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_unit_master',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_unit') }}
