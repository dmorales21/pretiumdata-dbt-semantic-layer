-- TRANSFORM.DEV.FACT_SE_YARDI_LEASE_HISTORY — read-through SOURCE_ENTITY.PROGRESS.YARDI_LEASE_HISTORY
-- Lease event history for deploy / stabilization pacing (Deploy).
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_lease_history',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_lease_history') }}
