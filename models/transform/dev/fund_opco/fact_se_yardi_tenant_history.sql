-- TRANSFORM.DEV.FACT_SE_YARDI_TENANT_HISTORY — read-through SOURCE_ENTITY.PROGRESS.YARDI_TENANT_HISTORY
-- Tenant residency / status history (Deploy).
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_tenant_history',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_tenant_history') }}
