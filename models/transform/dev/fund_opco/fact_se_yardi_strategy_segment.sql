-- TRANSFORM.DEV.FACT_SE_YARDI_STRATEGY_SEGMENT — read-through SOURCE_ENTITY.PROGRESS.YARDI_SEGMENTS
-- Strategy / segment taxonomy for allocation (Allocate tab).
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_strategy_segment',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_segments') }}
