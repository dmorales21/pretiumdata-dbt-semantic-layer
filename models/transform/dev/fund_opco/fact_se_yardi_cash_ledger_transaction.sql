-- TRANSFORM.DEV.FACT_SE_YARDI_CASH_LEDGER_TRANSACTION — read-through SOURCE_ENTITY.PROGRESS.YARDI_TRANS
-- Cash-oriented ledger transactions from SOURCE_ENTITY (Returns); not TRANSFORM.YARDI.TRANS_PROGRESS.
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_cash_ledger_transaction',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_trans') }}
