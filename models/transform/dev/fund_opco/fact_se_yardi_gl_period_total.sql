-- TRANSFORM.DEV.FACT_SE_YARDI_GL_PERIOD_TOTAL — read-through SOURCE_ENTITY.PROGRESS.YARDI_GLTOTAL
-- General-ledger rolled period totals for returns rollups (Returns).
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_gl_period_total',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_gltotal') }}
