-- TRANSFORM.DEV.FACT_SE_YARDI_GL_LINE_DETAIL — read-through SOURCE_ENTITY.PROGRESS.YARDI_GLDETAIL
-- GL line-level detail (Returns).
...
{{ config(
    materialized='view',
    alias='fact_se_yardi_gl_line_detail',
...
SELECT *
FROM {{ source('source_entity_progress', 'yardi_gldetail') }}
