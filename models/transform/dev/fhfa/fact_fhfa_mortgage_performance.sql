-- TRANSFORM.DEV.FACT_FHFA_MORTGAGE_PERFORMANCE — FHFA mortgage performance long-form series (SOURCE_SNOW.US_REAL_ESTATE).
{{ config(
    alias='fact_fhfa_mortgage_performance',
...
FROM {{ source('source_snow_us_real_estate', 'fhfa_mortgage_performance_timeseries') }} AS ts
INNER JOIN {{ source('source_snow_us_real_estate', 'fhfa_mortgage_performance_attributes') }} AS a
