...
| 11 | P1 | `pipeline` | *(mart spine)* — | Concept mart | **New model:** `concept_supply_pipeline_market_monthly.sql` prioritizing `TRANSFORM.DEV.FACT_MARKERR_RENT_LISTINGS_COUNTY_MONTHLY`, `TRANSFORM.DEV.FACT_CHERRE_VACANT_*_SNAPSHOT`, RCA pipeline / construction where `concept_code` already `pipeline` | Largest active MET family; mart filters out entity-level noise where possible. |
| 12 | P1 | `automation` | *(mart spine)* — | Concept mart | **New model:** `concept_workforce_task_automation_annual.sql` from `TRANSFORM.DEV.FACT_COUNTY_AI_REPLACEMENT_RISK`, `FACT_DOL_ONET_*`, Epoch crosswalk METs | After row 8, exclude mis-assigned First Street columns from this mart. |
...
**Missing vs ranked rows above:** `permits`, `pipeline`, `automation`, `vacancy`, `income`, `population`, `crime`, `education`, `rates`/`inflation` (optional national marts), `labor`, deeper `noi`/`ltv`/`cap_rate` market rolls if desired.
