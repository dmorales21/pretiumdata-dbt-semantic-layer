# Funnel Leasing — Data Guidance

⚠️ **INTERNAL — contract and operational context**

**Intake bundle:** `docs/vendors/funnel/` (machine column inventory + this narrative).  
**Canonical vendor hub:** `docs/vendor/funnel/` (dictionary + structured stubs).
...
| **vendor_id** | `VND_032` |
| **vendor_code** | `funnel` |
| **vendor_label** | Funnel Leasing |
| **definition** | Funnel property management platform data for BH Living covering leasing pipeline units applicants and online leasing transactions |
...
## Vendor methodology (full text from `docs/vendor/funnel/funnel.md`)

**Catalog row:** `vendor_id` = `VND_032` in `seeds/reference/catalog/vendor.csv`.

## 1. Identity

Funnel property management platform data for BH Living covering leasing pipeline units applicants and online leasing transactions
...
See [VENDOR_CONCEPT_COVERAGE_MATRIX.md](../migration/VENDOR_CONCEPT_COVERAGE_MATRIX.md) for **`funnel`** × concept × dataset gaps and stretch mappings.
...
|-----------|--------------|--------|-----------|------------|------------------|
| FUNNEL_MAN_B71B024A | pipeline | place | neutral | `SOURCE_ENTITY.BH` | `TBD` |
