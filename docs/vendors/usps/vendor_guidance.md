...
**Intake bundle:** `docs/vendors/usps/` (machine column inventory + this narrative).  
**Canonical vendor hub:** `docs/vendor/usps/` (dictionary + structured stubs).
...
| **vendor_code** | `usps` |
| **vendor_label** | USPS |
| **definition** | United States Postal Service address change and postal geography-adjacent feeds |
| **data_type** | administrative |
| **refresh_cadence** | monthly |
| **contract_status** | active |
| **source_schema** | `RAW.USPS` |
| **data_share_type** | s3 |
| **vertical_codes** | — |
...
## Vendor methodology (full text from `docs/vendor/usps/usps.md`)

**Catalog row:** `vendor_id` = `VND_047` in `seeds/reference/catalog/vendor.csv`.

## 1. Identity

United States Postal Service address change and postal geography-adjacent feeds
...
See [VENDOR_CONCEPT_COVERAGE_MATRIX.md](../migration/VENDOR_CONCEPT_COVERAGE_MATRIX.md) for **`usps`** × concept × dataset gaps and stretch mappings.
...
|-----------|--------------|--------|-----------|------------|------------------|
| USPS_MAN_5E66DD32 | pipeline | place | neutral | `RAW.USPS` | `TBD` |
