# Vendor: Zillow (`zillow`)

**Catalog row:** `vendor_id` = `VND_009` in `seeds/reference/catalog/vendor.csv`.

## 1. Identity

Zillow Home Value Index ZORI rent index and affordability series at MSA county city and ZIP grain
...
See [VENDOR_CONCEPT_COVERAGE_MATRIX.md](../migration/VENDOR_CONCEPT_COVERAGE_MATRIX.md) for **`zillow`** × concept × dataset gaps and stretch mappings.
...
Primary task / vet doc: [`migration/MIGRATION_TASKS_ZILLOW_TRANSFORM_DEV.md`](../migration/MIGRATION_TASKS_ZILLOW_TRANSFORM_DEV.md)
