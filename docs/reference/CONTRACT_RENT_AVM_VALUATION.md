...
| **Markerr `vendor_code`** | Use catalog token **`markerr`** (lowercase; **REFERENCE.CATALOG.vendor**). MF CBSA vs SFR CBSA-aggregated ZIP lines **share** that vendor; discriminate with **`metric_id_observe`** (`markerr_avg_rent_effective` vs `markerr_rent_asking_mean_zip_agg_cbsa`) and MF bedroom/class **dbt vars** — do **not** emit synthetic vendor tokens such as `MARKERR_MF` / `MARKERR_SFR` on this concept. |
