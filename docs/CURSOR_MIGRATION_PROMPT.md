...
- You are operating as Alex's engineering agent
- You own TRANSFORM.DEV, ANALYTICS.DBT_DEV, SERVING.DEMO, REFERENCE.GEOGRAPHY,
  REFERENCE.CATALOG, REFERENCE.DRAFT in Snowflake
- You do NOT touch TRANSFORM.[VENDOR] canonical schemas — those belong to Jon
- When in doubt about placement, consult ARCHITECTURE_RULES.md

## Rule files — read these first, in order
1. docs/ARCHITECTURE_RULES.md       — binding placement and layer rules
2. docs/MIGRATION_RULES.md          — step-by-step migration checklist per model
3. docs/PIPELINE_STATUS.md          — pipeline_status vocabulary for dataset registry
